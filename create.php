...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($input_salary)){
      ...
      // Validate localidad
    $input_localidad = trim($_POST["codloc"]);
    if(empty($input_localidad)){
        $localidad_err = "Please enter the localidad amount.";     
    } elseif(!ctype_digit($input_localidad)){
        $localidad_err = 'Please enter a positive integer value.';
    } else{
    ...
    if(empty($name_err) && empty($address_err) && empty($salary_err) && empty($localidad_err)){
        ...
        $sql = "INSERT INTO employees (name, address, salary, id_localidad) VALUES (?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_name, $param_address, $param_salary, $param_localidad);
